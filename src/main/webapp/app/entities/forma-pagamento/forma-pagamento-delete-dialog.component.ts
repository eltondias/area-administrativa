...
export class FormaPagamentoDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ formaPagamento }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(FormaPagamentoDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.formaPagamento = formaPagamento;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/forma-pagamento', { outlets: { popup: null } }]);
